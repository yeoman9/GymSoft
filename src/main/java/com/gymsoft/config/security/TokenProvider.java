...
import static com.gymsoft.config.security.Constants.ACCESS_TOKEN_VALIDITY_SECONDS;
import static com.gymsoft.config.security.Constants.AUTHORITIES_KEY;
import static com.gymsoft.config.security.Constants.SIGNING_KEY;
...
public class TokenProvider implements Serializable
{
    ...
    public String generateToken( Authentication authentication )
    {
        ...
        return Jwts.builder().setSubject( authentication.getName() ).claim( AUTHORITIES_KEY,
                                                                            authorities ).signWith( SignatureAlgorithm.HS256,
                                                                                                    SIGNING_KEY ).setIssuedAt( new Date( System.currentTimeMillis() ) ).setExpiration( new Date( System.currentTimeMillis() + ACCESS_TOKEN_VALIDITY_SECONDS * 1000 ) ).compact();
    }
